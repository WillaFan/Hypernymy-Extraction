...
logging.basicConfig(filename='trainLog.log', level=logging.DEBUG, format=LOG_FORMAT)
...
class EmbeddingTrainer():
    ...
    def load_data(self, filename, minimum_count=0, minimum_frequency=0):
        ...
        if self.verbose > 1:
            ...
            logging.info("Duplicate words removed.")
        ...
        if self.verbose > 1:
            ...
            logging.info("Index dictionary created.")
    ...
    def train(self, epochs=10, batch_size=32, lr=0.01, gpu=False, save_location=None):
        ...
        for epoch in range(epochs):
            ...
            for (hypernym, hyponym), count in self.data.items():
                for _ in range(count):
                    ...
                    if counter % batch_size == 0:
                        ...
                        # Compute cost
                        if gpu:
                            cost = self.net(hypernyms_ix.cuda(), hyponyms_ix.cuda(), counts_var.cuda(),                                            hypernyms1_ix.cuda(), hyponyms1_ix.cuda(), counts1_var.cuda())
                        else:                        
                            cost = self.net(hypernyms_ix, hyponyms_ix, counts_var, hypernyms1_ix, hyponyms1_ix, counts1_var)
                            

                        cost_epoch += torch.sum(cost).data.item()
                        optimizer.zero_grad()
                        if gpu:
                    ...
                    if counter % 1000 == 0:
                        if self.verbose > 1:
                            logging.info("Epoch {} of {}: {} of {} ({:.4f}%)".format(epoch + 1, epochs,
            ...
            if not batch.is_empty():
                ...
                cost_epoch += torch.sum(cost).data
                optimizer.zero_grad()
                if gpu:
                    cost.backward(torch.ones(cost.size()).cuda())
                else:
            ...
            if self.verbose:
                ...
                logging.info("Average cost in epoch {}: {}".format(epoch + 1, cost_epoch / counter))
