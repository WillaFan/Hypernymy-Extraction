...
import numpy as np
import torch
from projection.task9_scorer import average_precision, precision_at_k, mean_reciprocal_rank
...
class Evaluator:
    ...
    def get_p_at_k(self, k, gold_ids):
        self.model.eval()
        pAk_vals = []
        for q_ix in range(self.nb_queries):
            h_ids = gold_ids[q_ix]
            ...
            for i, pred_id in enumerate(pred_ids):
                if pred_id in h_ids:
                    is_correct[i] = 1
            pAk_vals.append(precision_at_k(is_correct, k, len(h_ids)))
        pAk = np.sum(pAk_vals)    
        return pAk
    
    def get_MRR(self, gold_ids):
        self.model.eval()
        MRR_vals = []
        for q_ix in range(self.nb_queries):
            h_ids = gold_ids[q_ix]
            ...
            for i, pred_id in enumerate(pred_ids):
                if pred_id in h_ids:
                    is_correct[i] = 1
            MRR_vals.append(mean_reciprocal_rank(is_correct))
        MRR = np.sum(MRR_vals)
        return MRR
    ...
    def write_predictions(self, path, candidates):
        ...
        with codecs.open(path, "w", encoding="utf-8") as f:
            for q_ix in range(self.nb_queries):
                ...
                pred_strings = [candidates[p].replace("_", " ") for p in pred_ids]
